        ...
        <div class="modal-body">
          ...
          <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            <li
              ...
              :class="{
                'border-primary': defaultMeter && aMeter.keyId == defaultMeter.keyId,
                'border-secondary': defaultMeter && aMeter.keyId != defaultMeter.keyId,
              }"
              role="presentation"
              v-for="aMeter in unit.meters.filter( x => (x.keyId != 0) && (x.keyId != null))"
              :key="aMeter.keyId"
            >
              <button
                @click="defaultMeter = aMeter"
                data-bs-toggle="tab"
                :data-bs-target="'#tabAdd' + aMeter.keyId"
                role="tab"
                type="button"
                class="btn"
              >
              ...
              <!-- <div class="dropdown">
                <button
                  class="btn btn-primary dropdown-toggle but-1"
                  type="button"
          ...
          >
            <div
              class="md-form tab-pane fade"
              :id="'tabAdd' + aMeter.keyId"
            ...
            >
              ...
              <div class="tabcontent" v-show="step == 1">
                <form v-show="aMeter.step1form == 1">
                  ...
                  <div
                    ...
                    v-show="aMeter.device.manufacturer_id != 0"
                  >
                ...
                <!-- Device Form -->
                <form v-show="aMeter.step1form == 2">
                  <div class="form-group">
                ...
                <!-- Manufacture Form -->
                <form v-show="aMeter.step1form == 3">
                  <div class="form-group">
              ...
              <div class="tabcontent" v-show="step == 2">
                <form v-show="aMeter.step2form == 1">
                  <div class="form-group" style="position: relative">
              ...
              <div class="tabcontent" v-show="step == 6">
               <AddGoogleMap @location_emitter="locator"/>
              </div>
                ...
                <div v-show="aMeter.step5form == 2">
                  ...
                  <div class="form-group row">
                    <div class="col-10" style="display: flex">
                      <div class="outer-picture" @click="pickFileNotInstall(index)" v-for="(pic, index) in aMeter.reason.picture" :key="index">
                        <img
                          :src="
                            pic
                          "
                        ...
                        <span class="badge badge-success">Upload</span>
                        <input
                          @change="onFileChangeNotInstall(index, $event)"
                          class="form-control img-up form-control-lg"
                          type="file"
                          :id="'fileUploadNotInstall' + index"
                          style="display: none"
                          ref="file"
                        />
                      </div>
                    ...
                    <div class="col-2">
                       <button class="btn" type="button" @click="addReasonImg(aMeter.keyId)"><i class="icon-plus"></i></button>
                    </div>
                  </div>
                  <div class="form-group">
                    <button
...
export default {
  ...
  methods: {
    ...
    async addDevice() {
      ...
      let resDevice = await this.network().allDevices();
      this.devices = [...resDevice];
      this.device.name = "";
    ...
    pickFileNotInstall(ind) {
      let fileUpload = document.getElementById("fileUploadNotInstall" + ind);
      if (fileUpload != null) {
        fileUpload.click();
      }
    ...
    onFileChangeNotInstall(ind, event) {
      let file = event.target.files[0];
      ...
      if (file) {
        ...
        reader.onload = async (e) => {
            console.log(e)
          let image = e.currentTarget.result;
          console.log(ind);
          for (var i = 0; i < this.unit.meters.length; i++) {
            if (this.unit.meters[i].keyId == this.defaultMeter.keyId) {
              console.log(this.unit.meters[i].reason.picture[ind])
              this.$set(this.unit.meters[i].reason.picture, ind, image);
              console.log(this.unit.meters[i].reason.picture[ind])
            }
          }
        };
    ...
    addReasonImg(keyId){
      console.log(keyId)
      let meter = this.unit.meters.filter((x) => x.keyId === keyId)


      meter[0].reason.picture.push("/images/placeholder.jpg");
      console.log(meter[0])
    },
    ...
    validate: function (e) {
      e.preventDefault();
      if (this.unit.building_id != 0 && this.unit.name) {
        this.addUnit();
      } else {
        this.errorsUnit = {
          building_id: [],
    ...
    async addUnit() {
      ...
      let res = await this.network().addUnit(unit);
      this.$refs.closeAddUnit.click();
      this.utility().presentSuccessToast("Units is added successfully!");
      console.log(res.unit.building_id);
      this.$parent.getUnits(res.unit.building_id);
      this.closeModal();

      if(res.unit.unit_meter.length > 0){
        res.unit.unit_meter.forEach(async (item)=> {
          this.NoficationData.to = localStorage.getItem("_fcm_token")
          const user = await this.sqlite().getActiveUser();
          this.NoficationData.data.username = user.name;
          console.log(this.NoficationData.data.username)
          this.NoficationData.data.message = "A meter is added with serial number: " + item.device_serial_number + " and on unit " + res.unit.name;

          let resNotify = await this.network().firebaseNotification(this.NoficationData)
          console.log(resNotify);
          if(resNotify == 1){
            this.$emit('notification_emitter', this.NoficationData);
            this.notificationReceiveData.message = this.NoficationData.data.message
            this.notificationReceiveData.reciever_id = item.meter_created_by
            let resSaveNotify = this.network().saveFirebaseNotification(this.notificationReceiveData);
            // this.$emit("showNotification_emitter", resSaveNotify)
            console.log(resSaveNotify.notification);
          }
        });
      }

    },
    deleteSelectedUnit(unit) {
      console.log(unit);
      let index = this.unit.meters.findIndex((x) => x.keyId == unit.keyId);
      if (index != -1) {
