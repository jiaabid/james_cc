<template>
  <div class="row mb-2">
    ...
    <div
      ...
      v-show="step == item.id"
    >
      <div class="card-1" style="height: auto">
                  ...
                  <td>
                    <div class="d-flex px-2 py-1">
                      <div class="d-flex flex-column justify-content-center">
                        <a class="mb-0 text-sm"
                          >{{ item.column1 }} {{ item.column2 }}
                          {{ item.column3 }} {{ item.column4 }}</a
                        >
                      </div>
                    </div>
                  </td>
...
export default {
  ...
  mounted() {
    this.eventbus().$on("openSlide", async (data) => {
      this.step = data;
      ...
      //   if (this.step == 1) {
      //     const res = await this.network().getPropertyList();
      //     this.list = res.properties.map((e) => {
      //       return e;
      //     });
      //     console.log({ res });
      //   }
  ...
  methods: {
    ...
    async getProperties() {
      const res = await this.network().getPropertyList();
      this.list = res.properties.map((element) => {
        element["column1"] = element.name;
        return element;
      });
    },
    async getUsers() {
      const res = await this.network().getUserList();
      this.list = res.users.map((element) => {
        element["column2"] = element.name;
        return element;
      });
    },
    async getBuildings() {
      const res = await this.network().getBuildingList();
      console.log({ res });
      this.list = res.buildings.map((element) => {
        element["column3"] = element.name;
        return element;
      });
    },
    async getMeters() {
      const res = await this.network().getMeterList();
      this.list = res.meters.map((element) => {
        element["column4"] = element.meter_name;
        return element;
      });
    },
  },
